...
A clean-room, cross-platform (pure-Python) reimplementation of Arvid Bux's
...
top of it — each opt-in or automatic, none breaking byte-faithful output:
...
Or, from the project folder on any OS:
```
...
* `-b` / `-l` (aperture supplied) — byte-identical, including lens EXIF.
* `.BIA` dark subtraction — byte-identical.
...
**m8raw2dng** by **Arvid Bux** (m8raw2dng.de), who discovered how to unlock the
...
This project is **not affiliated with, authorized, or endorsed by** Arvid Bux or
